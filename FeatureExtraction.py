...
columnsTitles = ['num_a', 'num_b', 'num_c', 'num_ç', 'num_d', 'num_e', 'num_f', 'num_g', 'num_h', 'num_i', 'num_j', 'num_k', 'num_l', 'num_m', 'num_n', 'num_ñ', 'num_o', 'num_p', 'num_q', 'num_r', 'num_s', 'num_t', 'num_u', 'num_v', 'num_w', 'num_x', 'num_y', 'num_z', 
                ...
                'num_oe', 'num_à', 'num_á', 'num_â', 'num_ä', 'num_è', 'num_é', 'num_ê', 'num_ë', 'num_ì', 'num_í', 'num_î', 'num_ï', 'num_ò', 'num_ó', 'num_ô', 'num_ö', 'num_ù', 'num_ú', 'num_û', 'num_ü',
                'num_triple_vowels', 'num_consonants', 'num_triple_vowels', 'num_triple_consonants', 'num_capital','max_length_word', 'mean_length_words',
                 ]
...
for language in languages:
    if not os.path.exists(file_directory + extractedFeatures_folder + language + '.csv'):
        ...
        for line in lines:
            ...
            num_à = line.lower().count('à')
            num_á = line.lower().count('á')
            num_â = line.lower().count('â')
            num_ä = line.lower().count('ä')
            num_è = line.lower().count('è')
            num_é = line.lower().count('é')
            num_ê = line.lower().count('ê')
            num_ë = line.lower().count('ë')
            num_ì = line.lower().count('ì')
            num_í = line.lower().count('í')
            num_î = line.lower().count('î')
            num_ï = line.lower().count('ï')
            num_ò = line.lower().count('ò')
            num_ó = line.lower().count('ó')
            num_ô = line.lower().count('ô')
            num_ö = line.lower().count('ö')
            num_ù = line.lower().count('ù')
            num_ú = line.lower().count('ú')
            num_û = line.lower().count('û')
            num_ü = line.lower().count('ü')

            num_vowels = num_a + num_e + num_i + num_o + num_u + num_oe + num_à + num_á + num_â + num_ä + num_è + num_é + num_ê + num_ë + num_ì + num_í + num_î + num_ï + num_ò + num_ó + num_ô + num_ö + num_ù + num_ú + num_û + num_ü
