...
class KalmanFilter:
    def __init__(self, initial_state, initial_covariance, transition_matrix, observation_matrix, process_noise_covariance, observation_noise_covariance, n_languages):
        ...
        initial_state = np.array([1/n_languages] * n_languages)  # Assuming equal probability for each language
        # Initial covariance matrix
        initial_covariance = np.eye(n_languages) * 0.01  # Assuming small uncertainty
        # Transition matrix (identity matrix as we don't have a dynamic model for language probabilities)
        transition_matrix = np.eye(n_languages)
        # Observation matrix (identity matrix as we directly observe the language)
        observation_matrix = np.eye(n_languages)
        # Process noise covariance (assuming small process noise)
        process_noise_covariance = np.eye(n_languages) * 0.001
        # Observation noise covariance (assuming small observation noise)
        observation_noise_covariance = np.eye(n_languages) * 0.001
